...
class BoxNavigation:
    ...
    def new_navigation(self):
        ...
        2. 根据 y0 值决定旋转方向：
           ...
           在旋转过程中如果检测到目标立方体，则立即停止旋转，并导航至目标立方体前方 (target_x+0.4, target_y)
        3. 如果上述旋转过程中未检测到目标立方体，则导航至 (5.0, -12.0)，然后反向旋转180度检测目标立方体，
           如果检测到，则导航至目标立方体前方 (target_x+0.4, target_y)；否则输出未检测到立方体的日志。
        """
        current_x, current_y, current_yaw = self.get_current_pose()
        if current_x is None:
        ...
        if current_y < -12:
            ...
            if result == "detected":
                ...
                rospy.loginfo("检测到目标立方体，直接导航至前方 (%.2f, %.2f)", target_x+0.4, target_y)
                self.go_to_position(target_x+0.4, target_y)
                return True
        ...
        if result == "detected":
            ...
            target_y = self.target_cube_position["y"]
            rospy.loginfo("检测到目标立方体，直接导航至前方 (%.2f, %.2f)", target_x+0.4, target_y)
            self.go_to_position(target_x+0.5, target_y)
            return True
        else:
